module RestInPeace.Tests.``Json Comparison``
...
let ``Property names are case sensitive`` () =
    ...
    (fun () -> referenceJson |> JsonValue.Parse |> Json.assertJsonSubset expectedJson |> ignore)
    |> should throw typeof<Exception>

    (fun () -> referenceJson |> JsonValue.Parse |> Json.assertJsonExact expectedJson |> ignore)
    |> should throw typeof<Exception>
