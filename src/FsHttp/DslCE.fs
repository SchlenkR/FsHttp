...
module R =
    type HttpBuilderBase with
        ...
        member this.Request(StartingContext, method, url) = request method url id

        // RFC 2626 specifies 8 methods
        [<CustomOperation("GET")>]
        member this.Get(StartingContext, url) = get url id

        [<CustomOperation("PUT")>]
        member this.Put(StartingContext, url) = put url id

        [<CustomOperation("POST")>]
        member this.Post(StartingContext, url) = post url id

        [<CustomOperation("DELETE")>]
        member this.Delete(StartingContext, url) = delete url id

        [<CustomOperation("OPTIONS")>]
        member this.Options(StartingContext, url) = options url id

        [<CustomOperation("HEAD")>]
        member this.Head(StartingContext, url) = head url id

        [<CustomOperation("TRACE")>]
        member this.Trace(StartingContext, url) = trace url id

        [<CustomOperation("CONNECT")>]
        member this.Connect(StartingContext, url) = connect url id

        [<CustomOperation("PATCH")>]
        member this.Patch(StartingContext, url) = patch url id
...
module H =
    type HttpBuilderBase with
        ...
        member this.Accept(context, contentType) = Dsl.H.accept context contentType id

        /// Character sets that are acceptable
        [<CustomOperation("AcceptCharset")>]
        member this.AcceptCharset(context, characterSets) = Dsl.H.acceptCharset context characterSets id

        /// Acceptable version in time
        [<CustomOperation("AcceptDatetime")>]
        member this.AcceptDatetime(context, dateTime) = Dsl.H.acceptDatetime context dateTime id

        /// List of acceptable encodings. See HTTP compression.
        [<CustomOperation("AcceptEncoding")>]
        member this.AcceptEncoding(context, encoding) = Dsl.H.acceptEncoding context encoding id
        ...
        member this.BearerAuth(context, token) = Dsl.H.bearerAuth context token id

        /// Authentication header using Basic Auth encoding
        [<CustomOperation("BasicAuth")>]
        member this.BasicAuth(context, username, password) = Dsl.H.basicAuth context username password id

        /// Used to specify directives that MUST be obeyed by all caching mechanisms along the request/response chain
        [<CustomOperation("CacheControl")>]
        member this.CacheControl(context, control) = Dsl.H.cacheControl context control id

        /// What type of connection the user-agent would prefer
        [<CustomOperation("Connection")>]
        member this.Connection(context, connection) = Dsl.H.connection context connection id

        /// An HTTP cookie previously sent by the server with 'Set-Cookie'.
        [<CustomOperation("Cookie")>]
        member this.SetCookie(context, name, value) = Dsl.H.cookie context name value id
        ...
        member this.SetCookieForPath(context, name, value, path) = Dsl.H.cookieForPath context name value path id
        ...
        member this.Date(context, date, id) = Dsl.H.date context date id

        /// Indicates that particular server behaviors are required by the client
        [<CustomOperation("Expect")>]
        member this.Expect(context, behaviors) = Dsl.H.expect context behaviors id

        /// Gives the date/time after which the response is considered stale
        [<CustomOperation("Expires")>]
        member this.Expires(context, dateTime) = Dsl.H.expires context dateTime id

        /// The email address of the user making the request
        [<CustomOperation("From")>]
        member this.From(context, email) = Dsl.H.from context email id

        /// Custom header
        [<CustomOperation("Header")>]
        member this.Header(context, key, value) = Dsl.H.header key value context id

        /// The domain name of the server (for virtual hosting), and the TCP port number on which the server is listening.
        /// The port number may be omitted if the port is the standard port for the service requested.
        [<CustomOperation("Host")>]
        member this.Host(context, host) = Dsl.H.host context host id
        ...
        member this.IfMatch(context, entity) = Dsl.H.ifMatch context entity id

        /// Allows a 304 Not Modified to be returned if content is unchanged
        [<CustomOperation("IfModifiedSince")>]
        member this.IfModifiedSince(context, dateTime) = Dsl.H.ifModifiedSince context dateTime id

        /// Allows a 304 Not Modified to be returned if content is unchanged
        [<CustomOperation("IfNoneMatch")>]
        member this.IfNoneMatch(context, etag) = Dsl.H.ifNoneMatch context etag id

        /// If the entity is unchanged, send me the part(s) that I am missing; otherwise, send me the entire new entity
        [<CustomOperation("IfRange")>]
        member this.IfRange(context, range) = Dsl.H.ifRange context range id

        /// Only send the response if the entity has not been modified since a specific time
        [<CustomOperation("IfUnmodifiedSince")>]
        member this.IfUnmodifiedSince(context, dateTime) = Dsl.H.ifUnmodifiedSince context dateTime id

        /// Specifies a parameter used into order to maintain a persistent connection
        [<CustomOperation("KeepAlive")>]
        member this.KeepAlive(context, keepAlive) = Dsl.H.keepAlive context keepAlive id

        /// Specifies the date and time at which the accompanying body data was last modified
        [<CustomOperation("LastModified")>]
        member this.LastModified(context, dateTime) = Dsl.H.lastModified context dateTime id

        /// Limit the number of times the message can be forwarded through proxies or gateways
        [<CustomOperation("MaxForwards")>]
        member this.MaxForwards(context, count) = Dsl.H.maxForwards context count id

        /// Initiates a request for cross-origin resource sharing (asks server for an 'Access-Control-Allow-Origin' response header)
        [<CustomOperation("Origin")>]
        member this.Origin(context, origin) = Dsl.H.origin context origin id

        /// Implementation-specific headers that may have various effects anywhere along the request-response chain.
        [<CustomOperation("Pragma")>]
        member this.Pragma(context, pragma) = Dsl.H.pragma context pragma id
        ...
        member this.Prefer(context, prefer) = Dsl.H.prefer context prefer id

        /// Authorization credentials for connecting to a proxy.
        [<CustomOperation("ProxyAuthorization")>]
        member this.ProxyAuthorization(context, credentials) = Dsl.H.proxyAuthorization context credentials id

        /// Request only part of an entity. Bytes are numbered from 0
        [<CustomOperation("Range")>]
        member this.Range(context, start, finish) = Dsl.H.range context start finish id
        ...
        member this.Referer(context, referer) = Dsl.H.referer context referer id
        ...
        [<CustomOperation("TE")>]
        member this.TE(context, te) = Dsl.H.te context te id

        /// The Trailer general field value indicates that the given set of header fields is present in the trailer of a message encoded with chunked transfer-coding
        [<CustomOperation("Trailer")>]
        member this.Trailer(context, trailer) = Dsl.H.trailer context trailer id
        ...
        member this.TransferEncoding(context, directive) = Dsl.H.transferEncoding context directive id

        /// Microsoft extension to the HTTP specification used in conjunction with WebDAV functionality.
        [<CustomOperation("Translate")>]
        member this.Translate(context, translate) = Dsl.H.translate context translate id

        /// Specifies additional communications protocols that the client supports.
        [<CustomOperation("Upgrade")>]
        member this.Upgrade(context, upgrade) = Dsl.H.upgrade context upgrade id

        /// The user agent string of the user agent
        [<CustomOperation("UserAgent")>]
        member this.UserAgent(context, userAgent) = Dsl.H.userAgent context userAgent id

        /// Informs the server of proxies through which the request was sent
        [<CustomOperation("Via")>]
        member this.Via(context, server) = Dsl.H.via context server id

        /// A general warning about possible problems with the entity body
        [<CustomOperation("Warning")>]
        member this.Warning(context, message) = Dsl.H.warning context message id

        /// Override HTTP method.
        [<CustomOperation("XHTTPMethodOverride")>]
        member this.XHTTPMethodOverride(context, httpMethod) = Dsl.H.xhttpMethodOverride context httpMethod id
...
module B =
    type HttpBuilderBase with
        ...
        member this.Body(context) = Dsl.B.body context id

        [<CustomOperation("binary")>]
        member this.Binary(context, data) = Dsl.B.binary context data id

        [<CustomOperation("stream")>]
        member this.Stream(context, stream) = Dsl.B.stream context stream id

        [<CustomOperation("text")>]
        member this.Text(context, text) = Dsl.B.text context text id

        [<CustomOperation("json")>]
        member this.Json(context, json) = Dsl.B.json context json id

        [<CustomOperation("formUrlEncoded")>]
        member this.FormUrlEncoded(context, data) = Dsl.B.formUrlEncoded context data id

        [<CustomOperation("file")>]
        member this.File(context, path) = Dsl.B.file context path id

        /// The type of encoding used on the data
        [<CustomOperation("ContentEncoding")>]
        member this.ContentEncoding(context, encoding) = Dsl.B.contentEncoding context encoding id

        /// The MIME type of the body of the request (used with POST and PUT requests)
        [<CustomOperation("ContentType")>]
        member this.ContentType(context, contentType) = Dsl.B.contentType context contentType id
...
module M =
    type HttpBuilderBase with
        ...
        member this.Multipart(context) = Dsl.M.multipart context id
        ...
        member this.ValuePart(context, name, value) = Dsl.M.valuePart context name value id

        [<CustomOperation("filePart")>]
        member this.FilePart(context, path) = Dsl.M.filePart context path id

        [<CustomOperation("filePartWithName")>]
        member this.FilePartWithName(context, name, path) = Dsl.M.filePartWithName context name path id

        /// The MIME type of the body of the request (used with POST and PUT requests)
        [<CustomOperation("ContentTypePart")>]
        member this.ContentTypePart(context, contentType) = Dsl.M.contentType context contentType id
...
module Config =
    ...
    type HttpBuilderBase with
        ...
        member this.Timeout(context, value) = Dsl.Config.timeout context value id

        [<CustomOperation("timeoutInSeconds")>]
        member this.TimeoutInSeconds(context, value) = Dsl.Config.timeoutInSeconds context value id

        [<CustomOperation("transformHttpRequestMessage")>]
        member this.TransformHttpRequestMessage(context, map) = Dsl.Config.transformHttpRequestMessage context map id

        [<CustomOperation("transformHttpClient")>]
        member this.TransformHttpClient(context, map) = Dsl.Config.transformHttpClient context map id

        [<CustomOperation("proxy")>]
        member this.Proxy(context, url) = Dsl.Config.proxy context url id
