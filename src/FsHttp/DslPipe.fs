...
module DslPipe =
    ...
    module Request =
        ...
        let get (url:string) =
            Dsl.Request.get url id
        
        let put (url:string) =
            Dsl.Request.put url id
        
        let post (url:string) =
            Dsl.Request.post url id
        
        let delete (url:string) =
            Dsl.Request.delete url id
        
        let options (url:string) =
            Dsl.Request.options url id
        
        let head (url:string) =
            Dsl.Request.head url id
        
        let trace (url:string) =
            Dsl.Request.trace url id
        
        let connect (url:string) =
            Dsl.Request.connect url id
        
        let patch (url:string) =
            Dsl.Request.patch url id
    ...
    module Header =
        ...
        let contentDisposition placement name fileName context =
            Dsl.Header.contentDisposition context placement name fileName id
        
        /// The type of encoding used on the data
        let contentEncoding encoding context =
            Dsl.Header.contentEncoding context encoding id
        
        /// The language the content is in
        let contentLanguage language context =
            Dsl.Header.contentLanguage context language id
        
        /// An alternate location for the returned data
        let contentLocation location context =
            Dsl.Header.contentLocation context location id
        
        /// A Base64-encoded binary MD5 sum of the content of the request body
        let contentMD5 md5sum context =
            Dsl.Header.contentMD5 context md5sum id
        
        /// Where in a full body message this partial message belongs
        let contentRange range context =
            Dsl.Header.contentRange context range id

        /// The date and time that the message was sent
        let date date context =
            Dsl.Header.date context date id
        
        /// Indicates that particular server behaviors are required by the client
        let expect behaviors context =
            Dsl.Header.expect context behaviors id
        
        /// Gives the date/time after which the response is considered stale
        let expires dateTime context =
            Dsl.Header.expires context dateTime id
        
        /// The email address of the user making the request
        let from email context =
            Dsl.Header.from context email id
        
        /// The domain name of the server (for virtual hosting), and the TCP port number on which the server is listening.
        /// The port number may be omitted if the port is the standard port for the service requested.
        let host host context =
            Dsl.Header.host context host id
        ...
        let prefer prefer context =
            Dsl.Header.prefer context prefer id
        
        /// Authorization credentials for connecting to a proxy.
        let proxyAuthorization credentials context =
            Dsl.Header.proxyAuthorization context credentials id
        
        /// Request only part of an entity. Bytes are numbered from 0
        let range start finish context =
            Dsl.Header.range context start finish id
        ...
        let referer referer context =
            Dsl.Header.referer context referer id
        ...
        let te te context =
            Dsl.Header.te context te id
        
        /// The Trailer general field value indicates that the given set of header fields is present in the trailer of a message encoded with chunked transfer-coding
        let trailer trailer context =
            Dsl.Header.trailer context trailer id
        ...
        let transferEncoding directive context =
            Dsl.Header.transferEncoding context directive id
        ...
        let translate translate context =
            Dsl.Header.translate context translate id
        
        /// Specifies additional communications protocols that the client supports.
        let upgrade upgrade context =
            Dsl.Header.upgrade context upgrade id
        
        /// The user agent string of the user agent
        let userAgent userAgent context =
            Dsl.Header.userAgent context userAgent id
        
        /// Informs the server of proxies through which the request was sent
        let via server context =
            Dsl.Header.via context server id
        
        /// A general warning about possible problems with the entity body
        let warning message context =
            Dsl.Header.warning context message id
        
        /// Override HTTP method.
        let xhttpMethodOverride httpMethod context =
            Dsl.Header.xhttpMethodOverride context httpMethod id

    [<AutoOpen>]
    module Body =

        let body headerContext =
            Dsl.Body.body headerContext id
        
        let text text context =
            Dsl.Body.text context text id

        let json json context =
            Dsl.Body.json context json id

        let formUrlEncoded data context =
            Dsl.Body.formUrlEncoded context data id

        /// The MIME type of the body of the request (used with POST and PUT requests)
        let contentType contentType context =
            Dsl.Body.contentType context contentType id
        ...
        let contentTypeWithEncoding contentTypeString charset context =
            Dsl.Body.contentTypeWithEncoding context contentTypeString charset id
