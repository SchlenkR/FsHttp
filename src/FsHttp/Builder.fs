...
module Builder =
    ...
    type HttpBuilder with
        ...
        member this.Request(StartingContext, method: HttpMethod, url: string) = Requests.request method url

        // RFC 2626 specifies 8 methods
        [<CustomOperation("GET")>]
        member this.Get(StartingContext, url: string) = Requests.get url
        
        [<CustomOperation("PUT")>]
        member this.Put(StartingContext, url: string) = Requests.put url
        
        [<CustomOperation("POST")>]
        member this.Post(StartingContext, url: string) = Requests.post url
        
        [<CustomOperation("DELETE")>]
        member this.Delete(StartingContext, url: string) = Requests.delete url
        
        [<CustomOperation("OPTIONS")>]
        member this.Options(StartingContext, url: string) = Requests.options url
        
        [<CustomOperation("HEAD")>]
        member this.Head(StartingContext, url: string) = Requests.head url
        
        [<CustomOperation("TRACE")>]
        member this.Trace(StartingContext, url: string) = Requests.trace url
    ...
    type HttpBuilder with
        ...
        member this.Accept (context: HeaderContext, contentType: string) = Headers.accept contentType context

        /// Character sets that are acceptable
        [<CustomOperation("AcceptCharset")>]
        member this.AcceptCharset (context: HeaderContext, characterSets: string) = Headers.acceptCharset characterSets context

        /// Acceptable version in time
        [<CustomOperation("AcceptDatetime")>]
        member this.AcceptDatetime (context: HeaderContext, dateTime: DateTime) = Headers.acceptDatetime dateTime context
        
        /// List of acceptable encodings. See HTTP compression.
        [<CustomOperation("AcceptEncoding")>]
        member this.AcceptEncoding (context: HeaderContext, encoding: string) = Headers.acceptEncoding encoding context
        
        /// List of acceptable human languages for response
        [<CustomOperation("AcceptLanguage")>]
        member this.AcceptLanguage (context: HeaderContext, language: string) = Headers.acceptLanguage language context
        
        /// The Allow header, which specifies the set of HTTP methods supported.
        [<CustomOperation("Allow")>]
        member this.Allow (context: HeaderContext, methods: string) = Headers.allow methods context
        
        /// Authentication credentials for HTTP authentication
        [<CustomOperation("Authorization")>]
        member this.Authorization (context: HeaderContext, credentials: string) = Headers.authorization credentials context
        
        /// Authentication header using Basic Auth encoding
        [<CustomOperation("BasicAuth")>]
        member this.BasicAuth (context: HeaderContext, username: string, password: string) = Headers.basicAuth username password context
        
        /// Used to specify directives that MUST be obeyed by all caching mechanisms along the request/response chain
        [<CustomOperation("CacheControl")>]
        member this.CacheControl (context: HeaderContext, control: string) = Headers.cacheControl control context
        
        /// What type of connection the user-agent would prefer
        [<CustomOperation("Connection")>]
        member this.Connection (context: HeaderContext, connection: string) = Headers.connection connection context
        ...
        member this.ContentDisposition (context: HeaderContext, placement: string, name: string option, fileName: string option) =
            Headers.contentDisposition placement name fileName context
        
        /// The type of encoding used on the data
        [<CustomOperation("ContentEncoding")>]
        member this.ContentEncoding (context: HeaderContext, encoding: string) = Headers.contentEncoding encoding context
        
        /// The language the content is in
        [<CustomOperation("ContentLanguage")>]
        member this.ContentLanguage (context: HeaderContext, language: string) = Headers.contentLanguage language context
        
        /// An alternate location for the returned data
        [<CustomOperation("ContentLocation")>]
        member this.ContentLocation (context: HeaderContext, location: string) = Headers.contentLocation location context
        
        /// A Base64-encoded binary MD5 sum of the content of the request body
        [<CustomOperation("ContentMD5")>]
        member this.ContentMD5 (context: HeaderContext, md5sum: string) = Headers.contentMD5 md5sum context
        
        /// Where in a full body message this partial message belongs
        [<CustomOperation("ContentRange")>]
        member this.ContentRange (context: HeaderContext, range: string) = Headers.contentRange range context
        ...
        member this.Date (context: HeaderContext, date: DateTime) = Headers.date date context
        
        /// Indicates that particular server behaviors are required by the client
        [<CustomOperation("Expect")>]
        member this.Expect (context: HeaderContext, behaviors: string) = Headers.expect behaviors context
        
        /// Gives the date/time after which the response is considered stale
        [<CustomOperation("Expires")>]
        member this.Expires (context: HeaderContext, dateTime: DateTime) = Headers.expires dateTime context
        
        /// The email address of the user making the request
        [<CustomOperation("From")>]
        member this.From (context: HeaderContext, email: string) = Headers.from email context
        
        /// The domain name of the server (for virtual hosting), and the TCP port number on which the server is listening.
        /// The port number may be omitted if the port is the standard port for the service requested.
        [<CustomOperation("Host")>]
        member this.Host (context: HeaderContext, host: string) = Headers.host host context
        ...
        member this.IfMatch (context: HeaderContext, entity: string) = Headers.ifMatch entity context
        
        /// Allows a 304 Not Modified to be returned if content is unchanged
        [<CustomOperation("IfModifiedSince")>]
        member this.IfModifiedSince (context: HeaderContext, dateTime: DateTime) = Headers.ifModifiedSince dateTime context
        
        /// Allows a 304 Not Modified to be returned if content is unchanged
        [<CustomOperation("IfNoneMatch")>]
        member this.IfNoneMatch (context: HeaderContext, etag: string) = Headers.ifNoneMatch etag context
